...
# NOTE: If any of the column names below still don't match, print df.columns.tolist()
# at the top of the app temporarily (st.write(df.columns.tolist())) to double check.
...
with tab4:
    ...
    df["Virality_Score"] = df["likes_count"] + (2 * df["comments_count"]) + (3 * df["shares_count"])
    top10 = df.nlargest(10, "Virality_Score")[["platform", "likes_count", "comments_count", "shares_count", "Virality_Score"]]
